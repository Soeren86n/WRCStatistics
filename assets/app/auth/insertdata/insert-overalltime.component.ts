...
export class InsertOveralltimeComponent implements OnInit {
  ...
  deleteStagetime(stagetime: Stagetime) {
    this.insertService.deleteOveralltime(stagetime).subscribe(
      (data) => {
  ...
  generateObjs() {
    this.getService.getRallyCar(this.rallyselected).subscribe((cars: Rallycar[]) => {
      ...
      this.insertService.insertoveralltime(stagetimeobj).subscribe(
        (data) => {
          this.notificationService.handleError(data.notification);
        },
        (error) => console.error(error),
  ...
  getStagetimes() {
    this.getService.getOveralltime(this.stageselected).subscribe((stages: Stagetime[]) => {
